...
//public class App
	...
	public static @TL.Op(urlPath ="*")void firstPage(TL tl){//TODO: after the development stage of bootstrapping , change the respond to get from dbTbl-storage the js code path: "aswan2017.bootStrap" ::= raw minimal js code to load LocalStorage "aswan2017.BootStrap" and execute or do a xhr of xhr-op:aswan2017.BootStrap
		tl.respond("text/html",
				"<html><head><script>window.onload=function bootstrap(){"
						+"	var path='aswan2017.files:sys.js',x=localStorage[path] \n"
	...
	@Override public List creationDBTIndices(TL tl){
		return Util.lst(
			Util.lst("int(24) PRIMARY KEY NOT NULL AUTO_INCREMENT"//no
				...
				,"varchar(255) DEFAULT NULL"//usr
				,"varchar(255) DEFAULT NULL"//domain
				,"int(11) NOT NULL"//pk
				,"enum('New','Update','Delete','Login','Logout','Log','Error')"//act
				,"text"//json
			)
...
CREATE TABLE `domain` (
  ...
  key(`usr`,`domain`,`entity`,`logTime`) -- ,  key(`usr`,`domain`,`entity`,`id`,`col`,`logTime`),  key(`domain`,`entity`,`col`,`val`(250),`logTime`),
