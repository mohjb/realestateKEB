...
public class Report1D extends Report1C {
	...
	final static String jspName="report1D.jsp";
	...
	public static void gGet(TL tl,E e)throws Exception{
		if ("resetLookup".equals(e.op)) {
			...
			TL.Util.mapSet(tl.response
					...
					, "Statistics", Statistics.toJson( null )
					, "jspName", jspName
					, "namesGovs", e.namesGovs);}

enum Statistics {
	count("count(*)","integer")
	,amount("sum(`"+DataTbl.C.price+"`)","currency")
	,avgMtr("sum(`"+DataTbl.C.price+"`)/sum(`"+DataTbl.C.area+"`)","currency")
	,maxMtr("max(`"+DataTbl.C.price+"`/`"+DataTbl.C.area+"`)","currency")
	,minMtr("min(`"+DataTbl.C.price+"`/`"+DataTbl.C.area+"`)","currency")
	,avgLand("avg(`"+DataTbl.C.area+"`)","number")
	,SumLand("sum(`"+DataTbl.C.area+"`)","number")
	,maxLand("max(`"+DataTbl.C.area+"`)","number")
	,minLand("min(`"+DataTbl.C.area+"`)","number");

	public String sql, format;
	Statistics(String p,String frmt){sql=p;format=frmt;}
	public int code(){return codeBase+ordinal();}
	static Statistics[]a=values();static final int codeBase=34;

	static List<Map<String,String>>
	toJson(List<Map<String,String>>l){
 ...
 public static void query(TL tl,E e)throws Exception{try {
	...
	try {
		...
		int currentName = -1, base = term ==
			Lbl.Term.aggregate ? 1 : (to - from + 1) * term.base;
		...
		while ( rs.next() ) {
			...
			for (int c = 0; c < n; c++)
				try {tbl[c][yr] = rs.getFloat(c + 3);}
					...
					tl.error(jspName + ":query:resultSet.next:for cols:c="+c, ex);}
		}
	}catch ( Throwable x ) {
