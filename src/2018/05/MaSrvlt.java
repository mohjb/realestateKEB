...
/**
 * Created by Vaio-PC on 5/26/2018.
 */
public class MaSrvlt extends HttpServlet {
...
public static class M extends Sql.Tbl {
	public static final String dbtName = "ma";
	@F public int id=-1,parent;
	@F public String key;
	@F public Date logTime;
	@F public Map m,a;
	@F public DType dtyp;
	@F public Object body;
	...
	public M(int id,int parent,String key) {
		this(id,parent,key,null,null,null,null,null);}

	public M(int id,int parent,String key,Map meta,Map accessControl,Object body,DType dataTyp,Date lt) {
		this.id=id;this.parent=parent;this.key=key;m=meta;dtyp=dataTyp;a=accessControl;logTime=lt==null?new Date():lt;}
...
/*	Object jba(){
		if(body!=null)
			return body;
		try {body = ba==null?null:
			Json.Prsr.parse(
			new InputStreamReader(
			new ByteArrayInputStream( ba ) ));
			return body;}catch(Exception ex){
			TL.tl().error(ex);
		}return null;}*/}

	Object body(Object v){
		if(v instanceof String){dtyp=DType.str;
			Object o=m==null?m:m.get("MSrvlt$M$body$js");
	...
	@Override Sql.Tbl load(ResultSet rs,CI[]a)throws Exception{
		int c=0;for(CI f:a)if(f!=C.body )v(f,rs.getObject(++c));else
		{DType t=dtyp!=null?dtyp:DType.str;
			body =null;
			switch ( t ){
				case byteArray:
					body =rs.getBytes( ++c );break;
				case date:
					body =rs.getDate( ++c );break;
	...
	@Override public Object valForSql(CI f){
		return f==C.body&&dtyp==DType.jos?ba():super.valForSql(f);}
	...
	@Override public List creationDBTIndices(TL tl) {
		...
		List indices=Util.lst("unique(`"+C.parent+"`,`"+C.key+"`)"
			...
			,Util.lst(Util.lst(C.body,10))
			,Util.lst(C.logTime))
			,rows=Util.lst(
			Util.lst(1,0,tl.now,"AppRoot",null,null,null),
			Util.lst(2,1,tl.now,"Groups",null,null,null),
			Util.lst(3,1,tl.now,"Users",null,null,null),
			Util.lst(4,3,tl.now,"moh"
	...
	public static class L extends Sql.Tbl{
		...
		public L(int i,String c,String x,A a,Object o,Date lt) {
			id=i;col=c;aux=x;act= a;body =o;logTime=lt;usr=TL.tl().usrId();}
		...
		@Override public List creationDBTIndices(TL tl){
			...
			List indices=Util.lst("unique(`"+C.id+"`,`"+C.logTime+"`)"
				,Util.lst(C.id,C.col,Util.lst(C.body,10))
	...
	/** by default everything is locked, all objects, all properties, all data, all ops
	 * user-groups grants permissions, specifying prop and op,
	 *
	 * the java-func "checkAccess" would
	 * 1. M$this.accessControl.grant.<prop>.<op>, is it true , or a date that didnt expire, or a server-side-js , or an array of two dates
	 * 2. if not present then iterate over each group in M$this.accessControl.requireGroupMembership,
	 *      for each group check group.m.grant.<prop>.<op>, is it true , or a date that didnt expire, or a server-side-js , or an array of two dates
	 * //3. if not present , check M$this.accessControl.inheretPermissions, if true, repeat step with parent instead of M$.this
	 * 4. if not present, throw an IllegalAccessException
	 *
	 *
	 * M$this.accessControl.requireGroupMembership:[<int:groupId>,,, ]
	 *
	 * tl.usr.M.accessControl.userGroupsMemberships:[<int:groupId>,,,]
	 *
	 * userGroup::=m:{ //id:<int:groupId>
	 *      name:<str:group-name>
	 *      members:{<int:userId>:<true or str:expiryDate >,,,}
	 *      inherit:<bool>      //check M$this.parent perm
	 *      grant:{
	 *          <str:prop>:{<str:op>:<true or date or server-side-js or two-dates(startPerm-date , expiryPerm-date)> ,,,}
	 *          ,,, //prop-name * means match all props except "m.accessControl" ; op-name * means match all ops
	 *      }
	 *  }
	 *
	 *  pipe-dream: in userGroup add feature/prop "revoke", that takes-away permission, a list of groupId's
	 * */
	boolean checkAccess(String prop,String op,TL tl){
		Object o;int uid=tl.usrId();//=tl!=null && tl.usr!=null?tl.usr.get("M"):null;
		//if(o instanceof M) {
		// M u = (M) o;
		// o = m == null ? null : m.get("accessControl");
		Map ac=a;M prnt=a!=null?null:loadBy(parent);
		while(prnt!=null&&ac==null){if(prnt.a!=null)
			ac=prnt.a;
			if(ac==null&&prnt.parent!=0&&prnt.id!=prnt.parent)
				prnt=loadBy(prnt.parent);}
		if(ac==null)
			return false;
		o = ac.get("grant");
		if(o instanceof Map) {
			Map grant=(Map)o;
			o=grant.get("expiry");
			if(o instanceof Date && ((Date)o).getTime()>tl.now.getTime()) {
				M x=isAccessGranted(grant,prop,op,uid,tl);
				if(x!=null)
					return true;
		}}
		o=ac.get("requireGroupMembership");
		if(!(o instanceof List))
			return false;
		List l=(List)o;
		int ix=-1,ln=l.size();do{ix=findGroupWithUser(l,uid,ix+1,tl);
			if(ix!=-1){o=l.get(ix);if(o instanceof M){
				M g=(M)o;
				o=g.m==null?null:g.m.get("grant");
				if(o instanceof Map&&
				isAccessGranted((Map)o,prop,op,uid,tl)!=null)
					return true;
			}}
		}while(ix!=-1&&ix<ln);
		return false;//throw new IllegalAccessException("Illegal Access");
	}

	static int findGroupWithUser(List l,int uid,int startIx,TL tl){
		for(;startIx<l.size();startIx++)
		{Object o=l.get(startIx);
			if(o instanceof Number){
				M g=loadBy(((Number)o).intValue());
				if(g!=null&&g.m!=null){
					o=g.m.get("members");
					if(o instanceof Map){
						Map m=(Map)o;
						...
						||(o instanceof Date && ((Date)o).getTime()>tl.now.getTime()))
							return startIx;
					}
				}
			}
	...
	Object isAccessGranted(Map grant,String prop,String op,int uid,TL tl){
		Object o=grant==null?null:grant.get(prop);
		if(!(o instanceof Map))
			return null;
		Map m = (Map) o;
		o=m.get(op);
		...
		if(o instanceof Boolean )
			return ((Boolean)o).booleanValue()?o:null;
		if(o instanceof Date)
			return ((Date)o).getTime()>tl.now.getTime()?o:null;
		if(o instanceof List){
			List l=(List) o;
			if(l.size() == 2 && l.get(0) instanceof Date && l.get(1) instanceof Date){
				long n=tl.now.getTime(),
					d0=((Date)l.get(0)).getTime(),
					d1=((Date)l.get(1)).getTime();
				return d0 < n && n<d1?o:null;
		}}if(o instanceof Map){
			/////////////////////////////////////////////////////////////////
		}
		return null;}

	M XisAccessGranted(Map grant,String prop,String op,int uid,TL tl){
		Object o=null;Map g = (Map) o,ac=null;
		o = g.get(prop);
		if(o instanceof Map) {
			Map p = (Map) o;
			o = p.get(op);
			javax.script.ScriptEngine e = null;
			if(o instanceof Map) {
				ac = (Map) o;
				...
				|| (o instanceof List && ((List) o).size() == 2
					&& ((List) o).get(0) instanceof Date
					&& ((Date) ((List) o).get(0)).getTime() < tl.now.getTime()
					&& ((((List) o).get(1) instanceof Date &&((Date)
						((List) o).get(1)).getTime() > tl.now.getTime()
				) || ((List) o).get(1) == null)
				))
					return this;
			}
	 ...
	 */
	@HttpMethod(useClassName = false,checkAccessOp= "")public static M
	...
	txt( @HttpMethod(prmLoadByUrl= true) M x
		...
		if(tl.usr!=null)//TODO: check permission
		{x.body(v);x.update(cols(C.dtyp,C.body));}
		return x;}
	...
	dt(  @HttpMethod(prmLoadByUrl= true) M x
		  ,@HttpMethod(prmBody = true) String v, TL tl) throws Exception {
		if(tl.usr!=null)//TODO: check permission
		{x.body(Util.parseDate(v));x.update(cols(C.dtyp,C.body));}
		return x;}
	...
	static javax.script.ScriptEngine eng(M t//String key
		...
		if(e == null && createIfNotInit) {
			...
			Object o=t.m==null?t.m:t.m.get("MSrvlt$M$body$js");
			if(o instanceof Boolean&&((Boolean)o).booleanValue())try{
				e.eval((String) t.body);}catch(Exception ex){
				tl.error(ex);
			}
		}if(e!=null)
			e.put("tl", tl);
	...
	call(@HttpMethod(prmLoadByUrl = true) M m,
		...
		boolean b=mmbr!=null&&mmbr.contains(".");
		Object o=b?e.eval(mmbr
			.replaceAll("\\("," openParanthises ")
			.replaceAll("\\{"," openBrace ")
			.replaceAll("="," equalSign ")
			.replaceAll(";"," semicolon ")
			.replaceAll(","," comma "))//	.split("\\.");
			:e.get(mmbr);
		if(o==null)
			throw new Exception("no member:"+mmbr);
		o=e.eval("typoeof("+mmbr+")!='function'");
		if(o instanceof Boolean && ((Boolean)o).booleanValue())
			throw new Exception("not a function:"+mmbr);
		e.put("args", args);
		return e.eval(mmbr+".apply(args)");}
	...
	prop(@HttpMethod(prmLoadByUrl= true) M x,
		...
		return prop(x,prop,val,C.m,tl);}//prop

	@HttpMethod	public static boolean
	ac(@HttpMethod(prmLoadByUrl= true) M x,
	   @HttpMethod(prmUrlPart= true) String prop,
	   @HttpMethod(prmBody = true) Object val,
	   TL tl) throws Exception {//TODO: check permission
		return prop(x,prop,val,C.a,tl);}//prop


	public static boolean prop(M x, String prop, Object val,C col,TL tl) throws Exception {//TODO: check permission
		if(x==null||prop==null||col==null||prop.length()==0)
			return false;
		Map m=col==C.a?x.a:x.m;
		if(m==null)x.v(col,m=new HashMap());
		String[]a=prop.split("\\.");
		String mmbr=a[0];
		Object o=m,u=null;
		...
		if(o!=null) {
			...
			L.l(x.id,col.toString(),prop,L.A.update,val);
			return x.update(cols(col))!=null;
		}return false;
	}//prop
	...
	dates(@HttpMethod(prmName= "from") Date f,
		...
		Object[]w=where( where(C.body,Co.ge),f
			,where(C.body,Co.le),t
...
static final String packageName = "dev201805"
	, SrvltName = packageName + ".MaSrvlt"
	, UrlPrefix = "/mas/";

static Map<String, Method> mth = new HashMap<String, Method>();

static void staticInit() {
	registerMethods( MaSrvlt.class);
...
@HttpMethod(checkAccessOp= "no") public static Map
...
public @interface HttpMethod {
	...
	String checkAccessOp() default "check";//boolean usrLoginNeeded() default true;
	String checkAccessProp() default "";//boolean usrLoginNeeded() default true;
...
@Override public void service(HttpServletRequest request, HttpServletResponse response) {
	...
	try {
		...
		String hm = tl.h.req.getMethod(),caOp=null,caProp=null;
		...
		if(httpMethodAnno!=null)
			caOp=httpMethodAnno.checkAccessOp();
		if("check".equals(caOp))
			caOp=op.getName();
		if(tl.usr == null && (httpMethodAnno == null || caOp!=null))//!"".equals(caOp)
			op = null;
		if(op != null) {
			...
			for(Annotation[] t : prmsAnno) try {
				...
				if(caProp==null&&"".equals(caProp=pp.checkAccessProp()))
					caProp=null;
				if(caOp!=null && caProp!=null){}
				Object o = null;
				if(pp != null && pp.prmUrlPart())
					...
					args[i] = m == null ? null : m.invoke(prmClss, tl,tl.h.url);    //M instance
...
public static void main(String[]args)throws Exception{
	...
	String[][]prms= {/*1stString is method
			...
			, 3rdString is body */
		{"MaSrvlt.login"	,"/mas/moh","{pw:'"+ MaSrvlt.Util.b64e("m")+"'}"},
		{"M.prop"		,"/mas/apps/page/x1/MSrvlt$M$data$js","true"},
		{"M.txt"		,"/mas/apps/page/x1","function jo(x){this.h=x;return x+2;}\nx={f:function f1(x){ok='f1'+x;},i:5,a:[1,2,[6,7],{a:1,b:2}],o:{x:'hi',f:function f2(x){ok='f2'+x;}}}\nf=function f0(x){ok=x;}"},
		{"M.call"		,"/mas/apps/page/x1/jo","[1,2]"},
		{"M.eval"		,"/mas/apps/page/x1","d=33"},
		{"M.load"		,"/mas/4","{}"},
		{"M.dates"		,"/mas/","{from:{class:'date',str:'2017/05/01'},to:{class:'date',str:'2019/05/01'}}"},
		{"M.delete"		,"/mas/apps/page/x2",""},
		{"get"			,"/mas/apps/page",""},//TODO: write test cases for each control-flow and each feature in "get"
		{"M.poll"		,"/mas/","0"},
		{"M.dt"			,"/mas/apps/page/xx","2018/02/14"},//{class:'date',str:'2018/02/14'}
		{"M.prop"		,"/mas/apps/page/xx/you.know","it"},
		//{"M.newChild"	,"/mas/apps/page"
		//		,"{m:{clientOutput:[]},dtyp:'str',body:'[4,5]'}"},//key:'pageX',
		//{"M.newChild"	,"/mas/apps/page/xx","{}"},
		//{"M.newChild"	,"/mas/apps/page/x1","{id:-1}"},
		//{"M.newChild"	,"/mas/apps/page/x2","{m:{name:'moh'}}"},//{"M.getKeys"	,"/mas/4","['xx']"},
		{"M.update"		,"/mas/apps/page/xx","{dtyp:'str',body:'textual content'}"},
		{"M.meta"		,"/mas/apps/page/xx","{ok:'yes'}"},
		{"M.cmnds"		,"/mas/","["
			+"{method:'M.newChild',url:'/mas/apps/page/x2',body:\"{key:'x2',m:{},body:''}\"}"
			+",{method:'M.update' ,url:'/mas/apps/page/xx',body:\"{body:'ok'}\"}"
			+",{method:'M.load'   ,url:'/mas/apps/page/xx'}"
			+",{method:'M.txt'    ,url:'/mas/apps/page/xx',body:\"cmnds\"}"
			+",{method:'M.meta'   ,url:'/mas/apps/page/xx',body:\"{cmnds:1}\"}"
			+",{method:'M.dt'     ,url:'/mas/apps/page/xx',body:\"{class:'date',str:'2017/05/31'}\"}"
			+",{method:'M.dates'  ,url:'/mas/',body:\"{from:{class:'date',str:'2017/01/31'},to:{class:'date',str:'2019/2/28 23:59:59'}}\"}"
			+",{method:'M.prop'   ,url:'/mas/apps/page/xx/ok.to',body:\"you\"}"
			+",{method:'M.getKeys',url:'/mas/apps/page/xx',body:\"['jo','ko']\"}"
			+",{method:'M.call'   ,url:'/mas/apps/page/xx/jo',body:\"[5,60]\"}"
			+",{method:'M.eval'   ,url:'/mas/apps/page/xx',body:\"z=17\"}"
			+",{method:'M.delete' ,url:'/mas/apps/page/x2'}"
			+",{method:'M.get'    ,url:'/mas/apps/page'}"
			+",{method:'M.poll'   ,url:'/mas/',body:\"0\"}"
			+"]"},
		{"MaSrvlt.logout","/mas/xx","{abc:123}"}
	};

	MaSrvlt x=new MaSrvlt();
